  ...
  def self.generate_edges(fs, graph, conditional)
    ...
    while !records.empty?
      records.each do |record|
        edge = {:start_node => record.in_reply_to_screen_name, :end_node => record.screen_name, :edge_id => record.twitter_id, :time => record.created_at, :curation_id => graph.curation_id, :graph_id => graph.id, :style => EdgeGenerator.derive_style_from_tweet(record), :analysis_metadata_id => graph.analysis_metadata_id}
        edges << edge
      end
      Edge.save_all(edges)
      edges = []
    ...
    while !records.empty?
      records.each do |record|
        edge = {:start_node => record.followed_user_name, :end_node => record.follower_user_name, :edge_id => record.followed_user_id, :time => record.created_at, :curation_id => graph.curation_id, :graph_id => graph.id, :style => "friendship", :analysis_metadata_id => graph.analysis_metadata_id}
        edges << edge
      end
      Edge.save_all(edges)
      edges = []
