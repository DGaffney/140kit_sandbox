class AudienceReport
  ...
  def self.run(analysis_metadata_id)
    ...
    graph_points = []
    graph_points << {:label => "total_impressions", :value => impressions, :curation_id => curation.id, :analysis_metadata_id => @analysis_metadata.id, :graph_id => audience_overview_graph.id}
