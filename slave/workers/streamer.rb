...
class Streamer < Instance
  ...
  attr_accessor :user_account, :username, :password, :created_at, :next_dataset_ends, :queue, :params, :datasets
...
debugger
  ...
  def clean_up_datasets
    ...
    finished_datasets = started_datasets.select {|d| U.times_up?(d.created_at.gmt+d.params.split(",").last.to_i) }
    if !finished_datasets.empty?
      puts "Finished collecting "+finished_datasets.collect {|d| "#{d.scrape_type}:\"#{d.params.split(",").first}\"" }.join(", ")
