...
module Crewait
  ...
  def self.go!(sql_method="insert")
    @@hash_of_hashes.each do |key, hash|
      hash.import_to_sql(key, sql_method)
    end
    @@hash_of_hashes = {}
    @@hash_of_next_inserts = {}
  end
  ...
  module HashMethods
    def import_to_sql(model_class, sql_method)
      ...
      sql = values.to_crewait_sql
  		...
  		while !sql.empty? do
  			query_string = "#{sql_method} into #{model_class} (#{keys.join(', ')}) values #{sql.shift}"
  			while !sql.empty? && (query_string.length + sql.last.length < 999_999)  do
  				query_string << ',' << sql.shift
  			end
        DataMapper.repository.adapter.execute(query_string)
  		end
    end
...
DataMapper::Model.append_extensions(Crewait::BaseMethods)
