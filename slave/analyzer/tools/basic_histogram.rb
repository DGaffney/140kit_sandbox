class BasicHistogram < AnalysisMetadata
  ...
  def self.generate_graph_points(frequency_set)
    require 'fastercsv'
    @graph_points = []
    @graphs = []
  ...
  def self.frequency_graphs(fs, graph, tmp_folder)
  ...
  # def self.send_email(recipient, subject, message_content, collection)
