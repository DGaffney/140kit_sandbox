  ...
  def route(metadata)
    ...
    when "ruby"
      ...
      if metadata.researcher.share_email && metadata.curation.researcher.share_email
        Notification.post("@#{metadata.researcher}: A job you requested on @#{metadata.curation.researcher.user_name}'s dataset just finished on #140kit: http://140kit.com/analytics/#{metadata.id}") 
      elsif metadata.researcher.share_email
        Notification.post("@#{metadata.researcher}: A job you requested just finished on #140kit: http://140kit.com/analytics/#{metadata.id}") 
      end
    else 
