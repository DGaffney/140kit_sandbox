class InteractionList < AnalysisMetadata
  ...
  def calculate_degrees(curation, conditional)
    debugger
    degrees = []
  ...
  def calculate_overview(curation)
    debugger
    ...
    overview[:total_distinct_retweets] = DataMapper.repository.adapter.select("select count(distinct(edge_id)) from edges where curation_id = #{curation_id} and style = 'retweet'")
    overview[:total_distinct_mentions] = DataMapper.repository.adapter.select("select count(distinct(edge_id)) from edges where curation_id = #{curation_id} and style = 'mention'")
    retweet_network = Graph.first_or_create(:title => "retweet_network", :style => "network", :analysis_metadata_id => @analysis_metadata.id, :curation_id => curation.id)
    mention_network = Graph.first_or_create(:title => "mention_network", :style => "network", :analysis_metadata_id => @analysis_metadata.id, :curation_id => curation.id)
