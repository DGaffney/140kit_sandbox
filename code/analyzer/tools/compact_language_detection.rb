class CompactLanguageDetection < AnalysisMetadata
  ...
  def self.run(analysis_metadata_id)
    ...
    graph = Graph.first_or_create(:title => "cld_value_comprehensive", :style => "language_detection", :analysis_metadata_id => @analysis_metadata.id, :curation_id => curation.id)
    graph.save!
    offset = 0
    limit = 20000
    tweets = Tweet.all({:limit => limit, :offset => offset, :fields => [:twitter_id, :language]}.merge(conditional))
    values = []
    while !tweets.empty?
      tweets.each do |tweet|
        values << {:graph_id => graph.id, :label => tweet.twitter_id, :value => self.detect_language_name(tweet.text), :analysis_metadata_id => @analysis_metadata.id, :curation_id => curation.id}
      end
      offset += limit
      tweets = Tweet.all({:limit => limit, :offset => offset, :fields => [:twitter_id, :language]}.merge(conditional))
  ...
  def self.detect_language_name(data)
    return $language_map.invert[CLD.detect_language(data)]
  end
end
