...
class InReplyToUserIdRetweetGraph < AnalysisMetadata
  ...
  def self.run(curation_id)
    ...
    BasicHistogram.generate_graphs([{:analysis_metadata_id => self.analysis_metadata(curation)&&self.analysis_metadata(curation).id, :style => "network_graph", :title => "conversational_tweets_entity_based"}.merge(options)], curation) do |fs, graph, conditional|
  ...
  def self.generate_edges(fs, graph, conditional)
    ...
    offset = 0
    Tweet.all(conditional.merge({:in_reply_to_user_id.not => 0, :fields => [:twitter_id], :unique => true, :limit => 1000, :offset => 0}))
