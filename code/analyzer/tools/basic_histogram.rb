  ...
  def self.frequency_graphs(fs, graph, conditional, path=ENV['TMP_PATH'])
    ...
    if block_given?
      ...
      if fs[:attribute].to_s == "created_at"
        ...
        while !results.empty?
          graph_points = results.collect{|record| {:label => record.send(fs[:attribute].to_s), :value => record.value, :graph_id => graph.id, :curation_id => graph.curation_id, :analysis_metadata_id => @analysis_metadata.id}}
